...
Version lives in **three** places that should be kept in sync: `pyproject.toml`, `frontend/package.json`, and the README's version references. At runtime it is read from the installed package metadata (`sql_dag_flow.__version__` via `importlib.metadata`), never hardcoded, and surfaced through `sql-dag-flow --version`, `GET /version` and the app footer. In a source checkout that reports whatever `pip install -e .` last recorded, so re-install after bumping.
...
- **`main.py`** — FastAPI app + CLI `start()` entrypoint (declared in `pyproject.toml` `[project.scripts]`). Holds server-global mutable state `CURRENT_DIRECTORY` (the project being analyzed) that endpoints mutate via `POST /config/path`. Finds a free port starting at 8000, opens the UI (as a standalone window via a Chromium browser's `--app` mode when one is installed — see `_open_app_window`; `--tab` or a missing browser falls back to `webbrowser.open`), mounts `static/` as an SPA with a catch-all route and no-cache headers. The three graph routes (`/graph`, `/graph/filtered`, `/graph/scoped`) are thin adapters over one `_graph_response()` helper — they differ only in how the request arrives, so put shared behavior there, not in the routes.
...
- **The source text decides what is a procedure, not the AST.** `_looks_like_procedure` / `_procedure_name_parts` match `CREATE [OR REPLACE] ... PROCEDURE` on the file with comments stripped, and override whatever sqlglot produced. sqlglot may return a `Create`, degrade the file to an opaque `Command`, or reject it outright depending on the body — none of which changes the declaration. Deciding the node type from the AST made an obvious fact depend on parser luck, and a procedure that fell through showed up as a plain table.
- **Procedure signatures need normalising before parsing.** sqlglot's BigQuery dialect rejects `OUT`/`INOUT`/`IN` parameter modes, which fails the *whole file* - the procedure then lands in the ParseError branch as type `unknown` with no dependencies, behind a misleading `Expecting )`. `parse_sql_files` retries a failed parse with `_normalize_procedure_signature()`, which strips the modes from the parameter list only. Downstream code must use `sql_for_parsing` (normalised); `sql_content` stays the original text for display and caching.
- **Procedure bodies must degrade gracefully.** BigQuery scripting (`DECLARE`, `SET`, `IF ... END IF`, `RAISE`, and `SELECT ... INTO vars`) is not modelled by sqlglot, and `sqlglot.parse()` is all-or-nothing: one unsupported statement raised and took the whole body with it. `_collect_statements` therefore falls back to `_parse_each`, which splits on semicolons (`_split_statements`, quote/backtick/comment aware) and parses each statement on its own, keeping whatever sqlglot understands. `_strip_select_into` drops the `INTO v1, v2` clause — it carries no lineage, only the `FROM` does — and is written to never match `INSERT INTO`. If even the shell fails, `_procedure_shell` parses the signature with a stub body so the procedure still becomes a real node instead of an `unknown` with a syntax error. **Never make body parsing all-or-nothing again.**
- **Stored procedures and multi-statement scripts.** `parsed` is only the *first* statement. `_collect_statements` returns the top-level statements plus, for a procedure, the statements inside its `BEGIN ... END` body — sqlglot cannot parse procedural bodies and degrades them to an opaque `Command`, so the body is lifted out with `_PROC_BODY_RE` and parsed as its own script. From those statements the parser derives `writes` (via `_statement_write_target`: INSERT/MERGE/UPDATE/DELETE/CREATE) separately from reads, and a table that is written is never also counted as read. `build_graph` turns `writes` into **outgoing** edges, which is what places a procedure between its inputs and outputs. `CALL` is found by regex (`_CALL_RE`) because sqlglot leaves it unparsed. sqlglot's logger is set to ERROR since its "falling back to Command" warnings are expected here.
...
1. **Persistent disk cache** — `.sqldagflow/cache.json` in the analyzed project, keyed per-file by `mtime_ns:size`, and versioned by `CACHE_VERSION`. Skips re-parsing unchanged files across restarts. **Failed parses are never cached**: the key is derived from the file, so a file that failed under an older parser would keep replaying that failure after the parser learned to handle it, and the fix would silently never reach the user. Bump `CACHE_VERSION` whenever a cached entry's shape or meaning changes. The `.sqldagflow` dir is skipped during the walk (hidden-dir filter). Cache stores JSON-safe copies (sets → lists).
2. **In-memory TTL cache** — `_parse_cache` in `main.py` (5s) dedupes rapid/concurrent requests. Invalidated on path change.

**Schema scoping is what makes the column passes affordable.** `qualify_columns` and `sqlglot.lineage` both scale with the size of the schema they are handed. Passing the whole project's schema cost ~440 ms per model; `_schema_for()` now builds a schema containing only the tables that model reads (plus itself), bringing it to ~19 ms — no less correct, since a model can only reference what it depends on. Two related rules: pass `lineage()` a **parsed AST**, never the raw SQL string (it re-parses the whole model for every column otherwise); and run every column type through `_sql_type_or_fallback()`, because `extract_output_columns` puts expression text in the type slot and sqlglot raises on it — that exception silently discarded the entire qualify pass for every model. Do not reintroduce a project-wide schema dict.
